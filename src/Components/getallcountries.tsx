...
const GetAllCountries = ({HandleClick, HandleChange, id}: Props) => {
  ...
  return (
    <div id={id} className=" w-full h-20 flex justify-center md:justify-start gap-6 md:ml-32 mt-32">
        ...
        <input id = "search" type="search" onChange={HandleChange} placeholder="search by country name" className={mode?'md:h-12 h-8 rounded p-1 bg-slate-300 ': 'md:h-12 h-8 rounded p-1'}  />
        <button
